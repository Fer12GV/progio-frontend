...
- **Siguiente en este repo** (prioridad **demo navegable por slices**, ver `docs/DEVPLAN.md`): **POC.7.1** (**AQUÍ ESTAMOS** — verificación login con roles seed). RBAC financiero operario (**POC.6.4** ✓: `EventTimeline` + `PrebillView`). Scroll táctil (**POC.6.3** ✓). **[BACKEND]** POC.4/5 sigue siendo el refuerzo ideal para datos reales y `operator_id` en listados.
